...
class LoGo(Strategy):
    def query(self, user_idx, label_idxs, unlabel_idxs, n_query=100):
        ...
        embedding = self.get_grad_embedding_maxInd(list(unlabel_idxs), net=l_net) #tensor of shape [num_samples, embedding_dim]
        ...
        for c_i in cluster_dict.keys():
            ...
            probs = self.predict_prob(cluster_idxs, g_net) # get probs from global model
        ...
        query_idx = list(set(query_idx)) #Choose the most uncertain sample (lowest index after sort) from that cluster.

        # Handle when fewer clusters than query budget
        if len(query_idx) != n_query:
            ...
            while len(query_idx) < n_query and retry_count < max_retries:
                for c_i in cluster_dict.keys():
                    ...
                    if len(cluster_idxs) < idx+1: # more uncertain samples from the same clusters by incrementing idx
                        skip.append(c_i)
                    else:
                        if U_dict[c_i] is None:
                            ...
                            U = (probs*log_probs).sum(1).numpy() # entropy
