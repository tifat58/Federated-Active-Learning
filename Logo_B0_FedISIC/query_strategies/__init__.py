...
def random_query_samples(dict_users_train_total, dict_users_test_total, args):
    ...
    dict_users_train_label_path = os.path.join(args.dict_user_path, f'dict_users_train_label_{args.current_ratio:.3f}.pkl')

    dict_users_train_label = {}

    total_clients = args.num_users
    total_data = len(args.dataset_train)

    # Calculate how many samples to initially query
    initial_labels_per_client = max(int(args.n_data / total_clients), 1)

    for user_idx in range(total_clients):
        user_total_data = list(dict_users_train_total[user_idx])

        # If a client has no data, safely handle it
        if len(user_total_data) == 0:
            print(f"Client {user_idx} has no data available!")
            dict_users_train_label[user_idx] = []
            continue

        # Ensure the client has enough samples for safe initial querying
        num_to_sample = min(initial_labels_per_client, len(user_total_data))
        
        sampled = random.sample(user_total_data, num_to_sample)
        dict_users_train_label[user_idx] = sampled

        print(f"Client {user_idx} initialized with {len(sampled)} samples.")
...
def algo_query_samples(dataset_train, dataset_query, dict_users_train_total, args):
    ...
    for user_idx in dict_users_train_total.keys():                
        ...
        #added
        if not new_data:
            print(f"[Client {user_idx}] Skipped querying due to empty query set.")
            continue
        #added
        ...
        #added
        if len(unlabel_idxs) < int(args.n_query / args.num_users):
            print(f"[Client {user_idx}] Skipping query due to insufficient unlabeled samples ({len(unlabel_idxs)} available).")
            continue
        #added
