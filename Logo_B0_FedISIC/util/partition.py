...
def partition_data_dirichlet(dataset_train, dataset_test, args):
    ...
    num_classes = len(np.unique([item[1] for item in dataset_train]))
    num_users = args.num_users
    beta = args.dd_beta

    def partition(dataset):
        labels = np.array([item['label'] if isinstance(item, dict) else item[1] for item in dataset_train]).flatten()
