...
def prepare_polyp():
    ...
    for i, site_name in enumerate(client_name):
        client_data_list.append(sorted(glob('/home/student_account/Desktop/Aditya/LoGo_FEAL_Segmentation/Dataset/FedPolyp/{}/image/*'.format(site_name))))
        client_mask_list.append(sorted(glob('/home/student_account/Desktop/Aditya/LoGo_FEAL_Segmentation/Dataset/FedPolyp/{}/mask/*'.format(site_name))))

    for client_idx in range(len(client_name)):
        if not os.path.exists('/home/student_account/Desktop/Aditya/LoGo_FEAL_Segmentation/data/FedPolyp/FedPolyp_npy/client{}'.format(client_idx+1)):
            os.makedirs('/home/student_account/Desktop/Aditya/LoGo_FEAL_Segmentation/data/FedPolyp/FedPolyp_npy/client{}'.format(client_idx+1))
        ...
        for data_idx in range(len(client_data_list[client_idx])):
            ...
            # np.save('/home/student_account/Desktop/Aditya/LoGo_FEAL_Segmentation/data/FedPolyp/FedPolyp_npy/client{}/sample{}.npy'.format(client_idx+1, data_idx+1), sample)
            sample_dict = {
                'image': img_np.astype(np.uint8),
                'mask': mask_np.squeeze().astype(np.uint8)  # squeeze 3rd dim for (H, W)
            }
            np.save('/home/student_account/Desktop/Aditya/LoGo_FEAL_Segmentation/data/FedPolyp/FedPolyp_npy/client{}/sample{}.npy'.format(client_idx+1, data_idx+1), sample_dict)
