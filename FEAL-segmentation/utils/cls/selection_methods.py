...
def fl_duc(global_model, local_model, dataloader, client_idx=0, round_idx=0):
    ...
    with torch.no_grad():
        for _, (_, data) in enumerate(dataloader):
            ...
            # Ensure block_features[-1] is a 4D tensor before processing
            if isinstance(block_features, list) and isinstance(block_features[-1], torch.Tensor) and block_features[-1].dim() == 4:
                l_feature = F.adaptive_avg_pool2d(block_features[-1], 3).flatten(start_dim=1)
                l_feature_list = torch.cat((l_feature_list, l_feature))
            else:
                # Skip this batch due to incompatible shape
                continue
            # l_feature_list = torch.cat((l_feature_list, l_feature))
...
def query_samples(al_method, global_model, local_model, tod_model, data_unlabeled, unlabeled_set, labeled_set, query_num, num_per_class, client_idx, round_idx, args):
    ...
    if al_method == 'Random':
        ...
        if u_dis_list.numel() == 0:
            logging.warning(f"[FEAL] Warning: Empty u_dis_list in round {round_idx}, client {client_idx}. Skipping query.")
            return list(range(query_num))  # fallback: random indices
