...
parser.add_argument('--local_epoch', type=int, default=2, help='number of local training epochs per round')
...
if __name__ == '__main__':
    ...
    assert dataset in ['FedISIC', 'FedPolyp']
                    ...
                    with open(os.path.join(snapshot_path, 'global_test_result.txt'), 'a') as f:
                        with open(os.path.join(snapshot_path, 'global_test_result.txt'), 'a') as f:
                            if dataset == 'FedISIC':
                                ...
                                print('client {}. Dice score:\t{}'.format(client_idx, metric), file=f)
                print('\n')
